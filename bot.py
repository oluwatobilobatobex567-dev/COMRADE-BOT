...
def db_upsert_movie(code: str, file_id: str, cover_id: str):
    conn = None
    try:
        conn = _pg_connect()
        with conn.cursor() as cur:
...
def db_get_movie(code: str) -> Optional[Tuple[str, str]]:
    ...
    try:
        conn = _pg_connect()
        with conn.cursor() as cur:
            cur.execute("SELECT file_id, cover_id FROM movies WHERE code = %s", (code,))
            row = cur.fetchone()
...
async def async_db_upsert_movie(code: str, file_id: str, cover_id: str):
    await asyncio.to_thread(db_upsert_movie, code, file_id, cover_id)


async def async_db_get_movie(code: str) -> Optional[Tuple[str, str]]:
    return await asyncio.to_thread(db_get_movie, code)
...
async def receive_cover(client: Client, message):
    ...
    try:
        await async_db_upsert_movie(code, file_id, cover_id)
...
async def cmd_start(client: Client, message):
    ...
    try:
        row = await async_db_get_movie(code)
    except Exception as e:
